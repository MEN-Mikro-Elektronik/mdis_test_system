...
function b_smb2_led_description {
    ...
    echo "DESCRIPTION:"
    echo "    Write to proper SMB register to power on LEDs"
    echo "    1.Load drivers: modprobe men_mdis_kernel"
    echo "    2.Get HW name to configure proper device address"
    echo "    3.Turn LEDs on by writing proper value to SMB register"
    echo "    4.Check if LEDs are on by reading proper value from SMB register"
    echo "    5.Turn LEDs off by writing proper value to SMB register"
    echo "    6.Check if LEDs are off by reading proper value from SMB register"
    echo "PURPOSE:"
