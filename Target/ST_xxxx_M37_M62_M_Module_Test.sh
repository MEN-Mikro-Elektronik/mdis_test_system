...
# This script performs tests on F205 with M37 and M62 M-Module.
...
while ${MachineRun}; do
        case $(echo "${MachineState}") in
          Step1)
                ...
                m_module_m37_test ${TestCaseLogName} ${TestCaseName} "1"
                CmdResult=$?
                if [ ${CmdResult} -ne ${ERR_OK} ]; then
          ...
          Step2)
                ...
                m_module_m62_test ${TestCaseLogName} ${TestCaseName} "1"
                CmdResult=$?
                if [ ${CmdResult} -ne ${ERR_OK} ]; then
