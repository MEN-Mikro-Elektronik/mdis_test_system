...
# This script performs tests on BL51E
...
while ${MachineRun}; do
        ...
        Step2)
                ...
                # Run step @2 Test CAN interface, there should be 1 CAN available
                can_test_ll_z15_loopback2 ${TestCaseLogName} "can_1"
                CmdResult=$?
                if [ "${CmdResult}" -ne "${ERR_OK}" ]; then
                         echo "can_test_ll_z15_loopback2 err: ${CmdResult} "\
                           | tee -a ${TestCaseLogName} 2>&1
                else
                         echo "can_test_ll_z15_loopback2 success "\
                           | tee -a ${TestCaseLogName} 2>&1
                fi
